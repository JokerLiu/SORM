...
public class TableContext {
	...
	static {
		try {// 初始化获得表的信息
			Connection con = DBManager.getConn();// 获取连接
			DatabaseMetaData dbmd = con.getMetaData();// 获取表信息

			ResultSet tableRet = dbmd.getTables(null, "%", "%", new String[] { "TABLE" });// 返回表结构信息

			while (tableRet.next()) {
				String tableName = (String) tableRet.getObject("TABLE_NAME");// 表名
				...
				while (set2.next()) {
					ColumnInfo ci2 = (ColumnInfo) ti.getColumns()
							.get(set2.getObject("COLUMN_NAME"));
	...
	// JavaFileUtils.createJavaPOFile(t, new TypeConvertor());
	// }
	// }
